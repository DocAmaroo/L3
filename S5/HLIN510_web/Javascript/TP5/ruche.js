    ...
    for( let l=0; l < nbLig; l++ ){
        ...
        */
        ...
        for ( let c=0; c < tmp_nbCol; c++ ){
            ...
            .style("fill", "white")
            .on("click", function(){
                let hexa = d3.select(this);
                let color = couleursJoueurs[joueurLocal];
                let actual_color = hexa.style("fill");
                if ( actual_color != color ){ hexa.style("fill", color); }
                else{ hexa.style("fill", "white"); }
                socket.emit("test", id, color);
            });
